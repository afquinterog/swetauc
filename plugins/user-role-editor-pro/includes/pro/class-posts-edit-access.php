<?php
use Festival\Service\FestivalService;
use Festival\Entity\FestivalEntity;
/*
...
class URE_Posts_Edit_Access {
    ...
    public function edit_user_posts_list($user) {
        ...
        if ( $this->lib->user_can_which( $user, $caps) ) {
            $festivals = FestivalService::getSingleton()->findBy(new FestivalEntity());
            
?>        
        <h3><?php _e('Posts/Pages Editor Restrictions', 'ure'); ?></h3>
        <script type="text/javascript">
            function setFestivalId(){
            	var idList = jQuery("#ure_posts_list").val();
            	var idListLength = idList.length;
            	var festivalId = jQuery("#festival_id").val();
            	...
            	if(festivalId !== "--"){
            		if(idListLength == 0){
            			...
            			idList = idList + "," + festivalId;
            		}
            		jQuery("#ure_posts_list").val(idList);
            	}
            }
        </script>
            ...
            <tr>
               ...
               <?php esc_html_e('edit posts/pages/custom post types with ID (comma separated) ', 'ure'); ?>
        			...
        			<td>
                        ...
                        <br>
                        <select id="festival_id" name="festival_id" onchange="setFestivalId()">
                            <option>--</option>
                            <?php foreach ($festivals as $festival): 
                                $festivalPost = get_post($festival->getId());
                                if($festivalPost != NULL):
                            ?>
                            <option value="<?php echo $festivalPost->ID; ?>"><?php echo $festivalPost->post_title; ?></option>
                            <?php 
                                endif;
                            endforeach; ?>
                        </select>
