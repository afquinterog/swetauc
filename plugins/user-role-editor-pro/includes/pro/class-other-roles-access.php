 ...
 * Class URE_Other_Roles_Access - prohibit selected access to the selected roles on per role/user basis
...
// reference to the code library object
    ...
    public function add_toolbar_buttons() {
        if (current_user_can('ure_other_roles_access')) {
?>                
        <button id="ure_other_roles_access_button" class="ure_toolbar_button" title="<?php esc_html__('Prohibit access to selected roles','ure');?>">Other Roles</button>
<?php
    ...
    public function add_js() {
        ...
        wp_localize_script( 'ure-other-roles-access', 'ure_data_other_roles_access',
                array(
                    ...
                    'dialog_title' => esc_html__('Other Roles Access', 'ure'),
                    'update_button' => esc_html__('Update', 'ure'),
                    'edit_users_required' => esc_html__('Turn ON "edit_users" capability to manage access of current role to other roles', 'ure')
                ));
    }
    // end of add_js()    
...
<?php        
    ...
    public function update_access() {
        ...
        if (!current_user_can('ure_other_roles_access')) {
            $this->notice = esc_html__('URE: you have not enough permissions to use this add-on.', 'ure');
            return;
        }
        ...
        if ($ure_object_type!=='role' && $ure_object_type!=='user') {
            $this->notice = esc_html__('URE: other roles access: Wrong object type. Data was not updated.', 'ure');
            return;
        }
        $ure_object_name = filter_input(INPUT_POST, 'ure_object_name', FILTER_SANITIZE_STRING);
        if (empty($ure_object_name)) {
            $this->notice = esc_html__('URE: other roles access: Empty object name. Data was not updated', 'ure');
            return;
        }
    ...
    protected function blocking_needed() {
        ...
        if ($this->lib->user_has_capability($current_user, 'ure_other_roles_access')) {
            return false;
        }
