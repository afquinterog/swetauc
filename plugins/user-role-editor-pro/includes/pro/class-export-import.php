...
<?php
    ...
    public function add_js() {
        ...
        wp_localize_script( 'ure-js-exp-imp', 'ure_data_exp_imp', 
                array(
                    'export_roles' => esc_html__('Export', 'ure'),
                    'import_roles' => esc_html__('Import', 'ure'),
                    'import_roles_title' => esc_html__('Import Roles', 'ure'),
                    'select_file_with_roles' => esc_html__('Select file with roles data', 'ure')
                ));
    }
    // end of add_js()
...
<?php        
    ...
    public function import_roles() {        
        ...
        if (current_user_can('ure_import_roles')) {        
            ...
            if (empty($upload_dir['error'])) {           
                ...
                $result->message = esc_html__('File upload error.', 'ure') .' '. $upload_dir['error'];
            }
        } else {
            $result = new StdClass();
            $result->success = false;
            $result->message = esc_html__('You do not have sufficient permissions to use this add-on.', 'ure');                        
        }
    ...
    protected function validate_role($role_id, $role) {
        ...
        if ($role_id!==$sanitized_role_id) {
            $result->message = esc_html__('Import failure: role ID contains invalid characters.', 'ure');            
        } elseif (!is_array($role)) {
            $result->message = esc_html__('Import failure: role should have an array structure.', 'ure');            
        } elseif (count($role)!==2) { // only two items 'name' and 'capabilities' should be in role array
            $result->message = esc_html__('Import failure: role '.$role_id.' array is not valid (> 2 items).', 'ure');            
        } elseif (!isset($role['name']) || !isset($role['capabilities'])) {   // wrong role array structure
            $result->message = esc_html__('Import failure: Wrong role '.$role_id.' array structure: name and capabilities not found.', 'ure');
        } elseif ($role['name']!==($name = sanitize_text_field($role['name']))) {  // wrong characters in the role name
            $result->message = esc_html__('Import failure: Wrong characters in the role name - sanitized version:', 'ure').' '.$name;        
        } elseif (!is_array($role['capabilities'])) {
            $result->message = esc_html__('Import failure: role capabilities should have an array structure.', 'ure');
        } else {
            $result->success = true;
        }
    ...
    protected function validate_capabilities($role_id, $capabilities) {
        ...
        foreach($capabilities as $key=>$value) {
            ...
            if ($key!==$sanitized_key) {    // illegial character found at the capability identifire
                $result->message = esc_html__('Import failure: Wrong characters in the capability ID - sanitized version:', 'ure').' '.$sanitized_key;
                break;
            } elseif (!is_bool($value) && value!=1 && value!=0) {
                $message = __('Import failure: Role "%s", wrong capability value for "%s" - only 1 and 0 are allowed, but "%s" found', 'ure');
    ...
    public function import_roles_notification() {
        if ($_REQUEST['action'] == 'roles_import_note') {             
            if ($_REQUEST['result']==1) {
                $this->lib->notification = esc_html__('Roles are imported successfully', 'ure');
            } elseif (!empty($_REQUEST['message'])) {
                $this->lib->notification = sanitize_text_field($_REQUEST['message']);
            } else {
                $this->lib->notification = esc_html__('Unknown error: Roles import was failed', 'ure');
            }
        }
    }
